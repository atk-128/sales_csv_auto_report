...
💡 **想定ユーザー**：請求書・売上チェックで「税抜/税込」を確認しながら、CSV集計とグラフを自動化したい人向け
...
このツールは「売上CSVの集計」をベースに、**実務（請求書チェック・売上確認）で使える形に拡張**した派生版です。

✅ **本ツールで増えたポイント（差分）**
- **税抜 / 税込の切り替え**：`--tax-rate 0.08` のように税率を明示して集計できる
- **複数CSVをまとめて集計**：月別/店舗別など、分割CSVを `input/` に入れるだけで結合
- **毎回の結果を履歴保存**：`output/report_YYYYMMDD_HHMMSS/` に分離して出力（比較・追跡が楽）
- **CSV + PNGを自動生成**：集計表だけでなく、日別売上・Top商品のグラフも同時に出る
...
- **税率を明示して集計**できるため、税抜/税込の混在ミスを防止
- **CSVベース出力**なので、Excel・スプレッドシートへ即再利用可能
- **毎回履歴が残る**ため、売上推移や再計算にも対応しやすい
